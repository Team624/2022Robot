...
public class PhotonVision extends CommandBase {
  ...
  public PhotonVision(Drivetrain drivetrainSubsystem,
    ...
    this.tower = tower;
    this.m_targetVision = targetVision;
  ...
  public void execute() {
    ...
    double xRot = m_targetVision.getYawVal();

    double radius = shooterVision.calculateActualDistance();

    if((Math.abs(xRot) < quickTurnTolerance)){
      quickTurnDone = true;
    }

    // If doing normal vision targeting
    if((Math.abs(xRot) < 500) && quickTurnDone){

      // For leds
      if ((Math.abs(xRot) < 4.0)){
      ...
      if (Math.abs(xRot) < visionResetTolerance && radius > 0){
      ...
      if (usedQuickTurn){
        thVelocity = getRotationPID(xRot);
