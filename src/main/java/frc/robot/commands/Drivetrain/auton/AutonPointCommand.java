...
public class AutonPointCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (!m_drivetrainSubsystem.stopAuton) {
      ...
      if (point < path.getLength() - 1) {
        double[] nearestPoint = getClosestPointOnLine(pathPoint.getX(), pathPoint.getY(),
            path.getPoint(point + 1).getX(), path.getPoint(point + 1).getY(), currentX, currentY);

        double velocityX = pathPoint.getVx()
            + (nearestPoint[0] - currentX) * Constants.Drivetrain.TRANSLATION_TUNING_CONSTANT;
        double velocityY = pathPoint.getVy()
            + (nearestPoint[1] - currentY) * Constants.Drivetrain.TRANSLATION_TUNING_CONSTANT;
  ...
  private void autonDrive(double xVelocity, double yVelocity, double theta) {
    ...
    double errorA = wantedAngle
        - m_drivetrainSubsystem.normalizeAngle(m_drivetrainSubsystem.getGyroscopeRotation().getRadians());
    ...
    m_drivetrainSubsystem.drive(ChassisSpeeds.fromFieldRelativeSpeeds(xVelocity, yVelocity, thVelocity, // In degrees
  ...
  private double[] getClosestPointOnLine(double point1X, double point1Y, double point2X, double point2Y, double point3X,
      double point3Y) {
  ...
  public boolean isFinished() {
    ...
    double distance = calculateDistance(currentX, currentY, path.getPoint(point + 1).getX(),
