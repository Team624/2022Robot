...
public class Intake extends SubsystemBase {
  ...
  private NetworkTableEntry AgitateSpeed = tab.add("Agitate Speed", Constants.Intake.agitateSpeed).withPosition(3, 1)
      .getEntry();
  ...
  public void checkNT() {
    ...
    if ((Iz != Iznew && Iznew != 0.0)) {
      intakePID.setIZone(Iznew);
      Iz = Iznew;
    }
    if ((FF != FFnew && FFnew != 0.0)) {
      intakePID.setFF(FFnew);
      FF = FFnew;
    }
